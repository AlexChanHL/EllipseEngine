...
class ModelList
{
   public:
    ...
    virtual void defineObject(const char* objectName, const char* texturePath, Ellipse::RenderObjData objectData) {
     objectData.textureData() = Ellipse::loadTexture(texturePath, false);
     addModelDefinition(objectName, "Assets/Shader/Quad.vert.glsl", "Assets/Shader/Quad.frag.glsl", objectData);
    ...
    virtual void addModel(const char* name, const char* objectName, Vec2 position) {
     addModel(name, objectName);
     translate(name, position);
    }
    ...
    ModelVal& model(const char* name) {
     if(!m_nameIds.contains(name)) {
      ELLIPSE_APP_LOG_WARN("name to id list does not contain id for: {}", name);
