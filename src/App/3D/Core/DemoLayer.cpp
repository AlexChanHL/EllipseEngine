...
void DemoLayer::init() {
   ...
   for(i32_t i=0;i<20;i++) {
    auto quad = Ellipse::RenderEntity2D(m_engine);
    quad.objectName() = "Quad";
    quad.onInit();
    i32_t id = 0;
    m_entitySystem.addEntity<Ellipse::RenderEntity2D>(quad, &id);
...
void DemoLayer::onUpdate(float dt)
{
    ...
    auto e = m_entitySystem.findEntity<Ellipse::RenderEntity>(m_entities[20]);
    if(e.id() != -1) {
    ...
    for(u32_t i=0; i<20;i++) {
     ...
     if(e.id() != -1) {
      time = time + (i * 0.05);
      pos = EllipseMath::Vec3(cos(time) * 0.8, sin(time) * 0.8, 0.0f);
      ...
      rComp.model()->scale(EllipseMath::Vec3(0.5f, 0.5f, 0.f));
