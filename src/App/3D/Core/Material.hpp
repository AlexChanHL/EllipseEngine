...
class Material
{
   public:
    ...
    void setAmbient(UniquePtr<EllipseMath::Vec3> ambient) { m_ambient = std::move(ambient); }
    void setDiffuse(UniquePtr<EllipseMath::Vec3> diffuse) { m_diffuse = std::move(diffuse); }
    void setSpecular(UniquePtr<EllipseMath::Vec3> specular) { m_specular = std::move(specular); }

    void setReflectiveness(UniquePtr<float> reflectiveness) { m_reflectiveness = std::move(reflectiveness); }

    UniquePtr<EllipseMath::Vec3>& ambient() { return m_ambient; }

    UniquePtr<EllipseMath::Vec3>& diffuse() { return m_diffuse; }

    UniquePtr<EllipseMath::Vec3>& specular() { return m_specular; }

    UniquePtr<float>& reflectiveness() { return m_reflectiveness; }
