...
namespace Ellipse {
 ...
 void OpenGLRenderPlugin::enable(u64_t cap) {
     glEnable(convertToGLCap(cap));
 }
 void OpenGLRenderPlugin::disable(u64_t cap) {
     glDisable(convertToGLCap(cap));
 }
 ...
 u32_t OpenGLRenderPlugin::convertToGLCap(u64_t cap) {
     switch(cap) {
      case DEPTH_TEST:
